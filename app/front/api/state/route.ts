...
function requireOwner(request: Request) {
  ...
  return userId && email === ownerEmail ? userId : null;
}
...
export async function GET(request: Request) {
  if (!requireOwner(request)) return Response.json({ error: "Sign in required." }, { status: 401 });
...
export async function POST(request: Request) {
  const actorId = requireOwner(request);
  ...
  try {
    const db = database();
    const statements = [db.prepare("INSERT INTO studio_operations (operation_id, actor_id, committed_at) VALUES (?, ?, ?)").bind(body.operationId, actorId, now), ...body.mutations.map((mutation) => db.prepare("INSERT INTO studio_state (source, entity_type, entity_id, field, value_json, revision, updated_at, updated_by) VALUES (?, ?, ?, ?, ?, 1, ?, ?) ON CONFLICT(source, entity_type, entity_id, field) DO UPDATE SET value_json = excluded.value_json, revision = studio_state.revision + 1, updated_at = excluded.updated_at, updated_by = excluded.updated_by").bind(mutation.source, mutation.entityType, mutation.entityId, mutation.field, JSON.stringify(mutation.value), now, actorId))];
