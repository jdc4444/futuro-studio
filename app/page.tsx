...
export default function Home({pilot=true}:{pilot?:boolean}={}){
 ...
 function returnHome(){
  setHover(false);
  if(cornerChangesAnimation){setCycle(current=>current+1);return;}
  setView('home');
 ...
 const resting=useMemo(()=>entries.map(e=>e.locale==='en'?{...e,text:'FUTURO',stroke:.007}:e),[entries]);   // Raleway Dots has one weight: a stroke of 0.7% of its size reads as 600 (the macro board's rule, (weight-400)/360 px at 80 px)
 ...
 return <div className={`futuro-site${pilot?' futuro-pilot':''}`} data-theme={theme} data-view={view} data-pilot-intro={pilotIntro} data-pilot-footage={pilotFootage} data-pilot-open={pilotOpen} data-backdrop={backdrop}>{pilot&&<Backdrop active={view==='home'&&pilotIntro&&!pilotOpen} skip={cycle} onShowing={setBackdrop}/>}<div className="futuro-hero" ref={hero}><div className="experience with-motion light-layout-center futuro-surface">
 ...
 <button className="futuro-information" aria-pressed={view==='information'} onClick={()=>openView('information')} disabled={status==='sending'}>About</button>
